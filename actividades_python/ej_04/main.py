...
rterm = 0 # Resistencia del termistor
...
while True:
    ...
    rterm = (r * LecturaTermVoltaje) / (vcc - LecturaTermVoltaje)

    # Conversión a temperatura usando la ecuación de Steinhart-Hart
    t = beta / (math.log(rterm / r) + (beta / t0))
    ...
    if TempPote > t:
        ...
        LEDAzul.value = diff / 5  # Brillo proporcional a la diferencia
        LEDRojo.value = 0         # Apaga el LED Rojo
    else:
        LEDAzul.value = 0         # Apaga ambos LEDs si no hay diferencia
        LEDRojo.value = 0
